  ...
  return instance => utils.memo(() => [instance.options.data], data => {
    ...
    const accessRows = function (originalRows, depth, parent) {
      ...
      for (let i = 0; i < originalRows.length; i++) {
        ...
        // }
        // Make the row
        const row$1 = row.createRow(instance, instance._getRowId(originalRows[i], i, parent), originalRows[i], i, depth); // Keep track of every row in a flat array

        rowModel.flatRows.push(row$1); // Also keep track of every row by its ID

        rowModel.rowsById[row$1.id] = row$1; // Push instance row into parent
        ...
        if (instance.options.getSubRows) {
          ...
          row$1.originalSubRows = instance.options.getSubRows(originalRows[i], i); // Then recursively access them
